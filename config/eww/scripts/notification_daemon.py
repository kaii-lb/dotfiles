        ...
        string = string + f"""
                  (button :class 'notification'
                   (box :orientation 'horizontal' :space-evenly false
                      (image :image-width 45 :image-height 45 :path '{item.icon or ''}')
                      (box :orientation 'vertical'
                        (label :limit-width 100 :halign "start" :justify "left" :wrap true :unindent true :text '{item.summary or ''}')
                        (label :limit-width 100 :halign "start" :justify "left" :wrap false :unindent true :text '{item.body or ''}')
                  )))
                  """
